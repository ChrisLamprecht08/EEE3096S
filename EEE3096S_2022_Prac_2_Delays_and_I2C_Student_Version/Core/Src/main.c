...
void setTime (uint8_t sec, uint8_t min, uint8_t hour, uint8_t dow, uint8_t dom, uint8_t month, uint8_t year)
{
	...
	//Question: how to get the parameters for the hal method
...
int epochFromTime(TIME time){
	...
	int time2 = ((((((time.year-2022)*365)+day)*24+time.hour)*60+time.minutes)*60+time.seconds);
	return EPOCH_2022 + time2;
}
